export default function Index() {
    return (
        <div className="text-center sm:w-96 m-auto">
            <div className="border-b-2 border-green-900 pb-4 mb-4">
                <h1 className="text-4xl mb-4">Howdy y'all 🤠</h1>
                <h2 className="text-2xl">Welcome to Seth Davis' Portfolio</h2>
            </div>
            <h3 className="text-2xl mb-4">A little about me</h3>
            <ul className="border-b-2 border-green-900 pb-4 mb-4">
                <li>Native Austinite</li>
                <li>TXST Alumni</li>
                <li>
                    UX Developer @ <a href="https://www.indeed.com/">Indeed</a>
                </li>
                <li>
                    Creator of{" "}
                    <a href="https://www.pseudoreact.com/">PseudoReact</a> ⚛️
                </li>
                <li>Electric vehicle enthusiast ⚡️</li>
                <li>Austin FC fan ⚽️</li>
                <li>Onewheel rider 🏂</li>
            </ul>
            <h3 className="text-2xl mb-4">Links</h3>
            <div className="flex flex-col sm:flex-row justify-center">
                <a
                    className="card w-full sm:w-auto mb-2 sm:mr-2"
                    href="https://github.com/sethdavis512"
                >
                    GitHub
                </a>
                <a
                    className="card w-full sm:w-auto mb-2 sm:mr-2"
                    href="https://codepen.io/sethdavis512"
                >
                    Codepen
                </a>
                <a
                    className="card w-full sm:w-auto mb-2"
                    href="https://twitter.com/sethdavis512"
                >
                    Twitter
                </a>
            </div>
        </div>
    );
}
