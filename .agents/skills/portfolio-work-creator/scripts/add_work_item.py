...
- Updates web/app/routes/work.tsx (adds to workItems array)
...
def add_to_work_page(work_file: Path, slug: str, title: str, description: str) -> None:
    """Add work item to work.tsx workItems array."""
    content = work_file.read_text()
    
    # Check if work item already exists
    if f"url: '/{slug}'" in content:
        print(f"⚠️  Work item '/{slug}' already exists in work.tsx")
        return
    
    # Find the workItems array
    work_items_match = re.search(r'const workItems = \[(.*?)\];', content, re.DOTALL)
    if not work_items_match:
        raise ValueError("Could not find workItems array in work.tsx")
    
    work_items_content = work_items_match.group(1)
    ...
    new_item = f"""    {{
        ...
        description:
            '{description}',
        imageSrc: '/{slug}-thumbnail.webp',
        imageAlt: '{title}',
        url: '/{slug}'
    }},"""
    
    # Insert after first item (Iridium)
    lines = work_items_content.split('\n')
    
    # Find the first closing brace after the first item
    insert_index = -1
    brace_count = 0
    found_first = False
    ...
    for i, line in enumerate(lines):
        ...
        if '}' in line:
            brace_count -= line.count('}')
            if brace_count == 0 and found_first:
    ...
    new_work_items = '\n'.join(lines)
    new_content = content.replace(work_items_content, new_work_items)
    
    work_file.write_text(new_content)
    print(f"✅ Added work item to work.tsx")


def main():
    """Main execution."""
    if len(sys.argv) < 2:
        print("Usage: python add_work_item.py <slug> [--title 'Work Title'] [--description 'Brief description'] [--no-prerender] [--no-work-page]")
        print("\nExample:")
        print("  python add_work_item.py my-awesome-project --title 'My Awesome Project' --description 'A cool project'")
        print("  python add_work_item.py another-project --no-prerender --no-work-page")
        sys.exit(1)
    ...
    try:
        ...
        work_file = root / "web" / "app" / "routes" / "work.tsx"
        ...
        if add_to_work:
            add_to_work_page(work_file, slug, title, description)
        else:
            print("⏭️  Skipped work.tsx (--no-work-page)")
